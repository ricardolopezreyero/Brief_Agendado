...
export function heroHeader(opts: { eyebrow: string; titulo: string; subtitulo: string; ancho?: boolean; volverHref?: string }): string {
  const volver = opts.volverHref ? `<a class="volver-header" href="${escapeHtml(opts.volverHref)}">← Volver</a>` : '';
  return `<header>
    <div class="brand"><img src="${LOGO_SUPERLEADS}" alt="SuperLeads"></div>
    ${volver}
    <div class="hero-text${opts.ancho ? ' ancho' : ''}">
